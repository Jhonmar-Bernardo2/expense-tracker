...
class UpsertApprovalVoucherRequest extends FormRequest
{
        ...
        $validator->after(function (Validator $validator): void {
            ...
            if ($module === ApprovalVoucherModule::Allocation && $action === ApprovalVoucherAction::Delete) {
                $validator->errors()->add(
                    'action',
                    'Monthly budget removal requests are no longer supported.',
                );
            }
    ...
    private function validateAttachmentRules(Validator $validator): void
    {
        ...
        if ($removeAttachmentIds->isNotEmpty()) {
            $matchedAttachmentCount = app(ApprovalVoucherAttachmentRepository::class)
                ->countMatchingSupportingDocumentsForVoucher(
                    $approvalVoucherId,
        ...
        $existingAttachmentCount = $approvalVoucherId > 0
            ? app(ApprovalVoucherAttachmentRepository::class)->countSupportingDocumentsForVoucher(
                $approvalVoucherId,
                $removeAttachmentIds->all(),
            )
            : 0;
